    ...
    def transform_query(
        ...
        try:
            ...
            transformation_prompt = (
                ...
                "- Prioritize content-rich chunks over header-only matches\n"
                "- Avoid literal date strings that only match "
                "metadata headers\n"
                f"- Example: 'What's new today?' → 'NVIDIA GPU computing, AI, "
                f"CUDA, machine learning, and deep learning developments from {current_month_year}'\n"
                f"- Example: 'Recent developments' → 'Latest NVIDIA GPU technology, "
                f"AI frameworks, CUDA programming, TensorRT, and deep learning updates from {current_month_year}'\n"
                f"- Example: 'Latest articles' → 'Most recent NVIDIA blog posts about "
                f"GPU computing, AI, machine learning, CUDA programming, performance optimization, and developer tools from "
                f"{current_month_year}'\n"
                f"- Example: 'Blogs from December 11, 2025' → "
                f"'NVIDIA GPU computing, AI, CUDA, TensorRT, machine learning, deep learning, optimization, and performance blog posts from December 2025'\n"
                f"- Example: 'December 2025 blogs' → 'NVIDIA technology developments "
                f"in GPU computing, AI, CUDA programming, neural networks, optimization, and inference from "
                f"December 2025'\n"
                f"- NEVER just transform 'Latest articles from December 2025' to 'articles from December 2025'\n"
                f"- ALWAYS add what KIND of articles (GPU, AI, CUDA, performance, etc)\n\n"
                f"ORIGINAL QUERY:\n{original_query}\n\n"
